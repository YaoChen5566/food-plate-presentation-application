...
void QtGuiApplication1::updateShowTable(QTableWidget *table) {
	...
	for (iterSelect = foodSelectedIndex.Element.begin(); iterSelect != foodSelectedIndex.Element.end(); iterSelect++) {
		...
		QPixmap pixelImage = QPixmap::fromImage(QImage((uchar*)modifyFoodImg[foodTmp.fileName].data, modifyFoodImg[foodTmp.fileName].cols, modifyFoodImg[foodTmp.fileName].rows, QImage::Format_ARGB32));
	...
	if (recommendOrNot == 1) {
		/* add recommend */
		food foodTmp = totalFood[nameFood.Element[recommendFood][0]];
...
void QtGuiApplication1::QTreeWidgetClicked_Recipe(QTreeWidgetItem* item, int column) {
	if (item->parent()) {
		...
		if (iterFindFood != foodSelectedIndex.Element.end()) { // already in food Select Index
			...
			for (int tt = 0; tt < nameFood.Element[tmpF.name].size(); tt++) {
				tmpFoodVec.push_back(totalFood[nameFood.Element[tmpF.name][tt]]);
			}
	...
	: QMainWindow(parent)
{
	...
	for (int i = 0; i < totalFood.size(); i++) {
		...
		nameFood.Element[totalFood[i].name].push_back(i);
	...
	for (int i = 0; i < RecipeFiles.size(); i++) {
		...
		for (int j = 0; j < RecipeFiles[i].size; j++) {
			...
			int f = nameFood.Element[recFoodName][0];
			
			cateRecipe[recipeFileName[i]].push_back(totalFood[f]);
			QTreeWidgetItem *leaf = new QTreeWidgetItem(root, QStringList() << QString::fromStdString(totalFood[f].name));
			QPixmap pixelImage = QPixmap::fromImage(QImage((uchar*)modifyFoodImg[totalFood[f].fileName].data, modifyFoodImg[totalFood[f].fileName].cols, modifyFoodImg[totalFood[f].fileName].rows, QImage::Format_ARGB32));
			QPixmap resPix = pixelImage.scaled(80, 80);
			leaf->setIcon(0, QIcon(resPix)); // set icon

			QString html = getToolTipQString(totalFood[f]);
	...
	for (int i = 0; i < totalFood.size(); i++)
		for (int j = 0; j < category.size(); j++)
			if (totalFood[i].category == category[j])
				cateFood.Element[category[j]].Element[totalFood[i].name].push_back(totalFood[i]);
	...
	for (int i = 0; i < category.size(); i++) {
		...
		for (iterFood = cateFood.Element[category[i]].Element.begin(); iterFood != cateFood.Element[category[i]].Element.end(); iterFood++)
		{
			QTreeWidgetItem *leaf = new QTreeWidgetItem(root, QStringList() << QString::fromStdString(iterFood->second[0].name));
			QPixmap pixelImage = QPixmap::fromImage(QImage((uchar*)modifyFoodImg[iterFood->second[0].fileName].data, modifyFoodImg[iterFood->second[0].fileName].cols, modifyFoodImg[iterFood->second[0].fileName].rows, QImage::Format_ARGB32));
			QPixmap resPix = pixelImage.scaled(80, 80);
			leaf->setIcon(0, QIcon(resPix)); // set icon
...
void QtGuiApplication1::compareWithRecipe(Mat& userDraw, recipe& rec2) {
	...
	for (int i = 0; i < groupIdx.size(); i++) {
		...
		cout <<contourIdx<<" ~ "<< foodCandidate.Element[contourIdx].Element.size() << endl;
		...
		for (int g = 1; g < groupIdx[i].size(); g++) {
			...
			for (int j = 0; j < foodCandidate.Element[contourIdx2].Element.size(); j++) {
				...
				cout << "w size: " << groupWarpMatOfContour[contourIdx2].size() << "__" << contourIdx2 << endl;
...
vector<int> getStrNutrient(string str, vector<fragList>& sortedFragList, recipe& recipeTmp) {
	...
	for (int i = 0; i < vec.size(); i++) {
		...
		for (int j = 0; j < recipeTmp.foodName().size(); j++) {
			int recipeIdx = nameFood.Element[recipeTmp.foodName()[j]][0];
...
void doCompare(bool isReverse, fragList& pairSeq, Mat& userDraw, vector<vector<Mat> >& descriptor, vector<vector<Point> >& sampleP, int contourIdx, int recipeIdx, recipe& rec) {
	...
	for (int f = 0; f < nameFood.Element[foodName].size(); f++)
	{
		int foodImgIdx = nameFood.Element[foodName][f];
		...
		cout << contourIdx << ": " << totalFood[nameFood.Element[foodName][f]] .name<<": "<< totalFood[nameFood.Element[foodName][f]].fileName << ": " << tmpPairSeq.Element.size() << endl;
